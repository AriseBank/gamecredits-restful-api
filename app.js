...
// This file contains the application code for the GameCredits Restful API
...
// Required Libraries For GameCredits Restful API. These are mandatory to interact with the RPC-JSON server.
...
const thePort = parseInt(process.env.APP_PORT) || 40001;
...
_.each(commands, (value, cmd) => {
    const config = {
        handler: function (req, res) {
            ...
            // Return the response from the GameCredits server
            return res;
        }
    };
...
// This function will start your GameCredits Sunrise API server. 
server.start((err) => {
    if (err)
        throw err;

    console.log('GameCredits Sunrise API server started at: ' + server.info.uri);
});
