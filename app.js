...
// This file contains the application code for the Zcash Restful API
...
// Required Libraries For Zcash Restful API. These are mandatory to interact with the RPC-JSON server.
...
_.each(commands, (value, cmd) => {
    const config = {
        handler: function (req, res) {
            ...
            // Return the response from the Zcash server
            return res;
        }
    };
...
// This function will start your Zcash Sunrise API server. 
server.start((err) => {
    if (err)
        throw err;

    console.log('Zcash Sunrise API server started at: ' + server.info.uri);
});
